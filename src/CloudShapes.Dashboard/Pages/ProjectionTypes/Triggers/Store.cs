...
/// <param name="monacoEditorHelper">The service used to to facilitate the Monaco Triggers configuration</param>
/// <param name="jsonSerializer">The service used to serialize/deserialize data to/from JSON</param>
/// <param name="yamlSerializer">The service used to serialize/deserialize data to/from YAML</param>
public class ProjectionTypeTriggersStore(ILogger<ProjectionTypeTriggersStore> logger, ICloudShapesApiClient cloudShapesApi, IMonacoEditorHelper monacoEditorHelper, IJsonSerializer jsonSerializer, IYamlSerializer yamlSerializer)
    : ComponentStore<ProjectionTypeTriggersState>(new())
{
    #region Selectors
    /// <summary>
    /// Gets an <see cref="IObservable{T}"/> used to observe <see cref="ProjectionTypeTriggersState.Status"/> changes
    /// </summary>
    public IObservable<string> Status => this.Select(state => state.Status).DistinctUntilChanged();

    /// <summary>
    /// Gets an <see cref="IObservable{T}"/> used to observe <see cref="ProjectionTypeTriggersState.ProjectionTypeName"/> changes
    /// </summary>
    public IObservable<string?> ProjectionTypeName => this.Select(state => state.ProjectionTypeName).DistinctUntilChanged();

    /// <summary>
    /// Gets an <see cref="IObservable{T}"/> used to observe <see cref="ProjectionTypeTriggersState.ProjectionType"/> changes
    /// </summary>
    ...
    #region Setters
    /// <summary>
    /// Sets the state's <see cref="ProjectionTypeTriggersState.ProjectionType"/>
    /// </summary>
    ...
    /// <summary>
    /// Sets the state's <see cref="ProjectionTypeTriggersState.ProjectionTypeName"/>
    /// </summary>
